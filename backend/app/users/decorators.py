...
def register_payload(func):
    return create_payload_decorator(
        func, validation.RegisterSchema, error_msg="Invalid register data"
    )


def reset_password_payload(func):
    return create_payload_decorator(
        func, validation.ResetPasswordSchema, error_msg="Invalid reset password data"
    )
...
def change_password_payload(func):
    return create_payload_decorator(
        func, validation.ChangePasswordSchema, error_msg="Invalid change password data"
    )


def change_details_payload(func):
    return create_payload_decorator(
        func, validation.ChangeDetailsSchema, error_msg="Invalid change details data"
    )
